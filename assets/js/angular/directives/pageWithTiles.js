      ...
      _(pwtCntrl.modelMeta.refs).forEach(function(ref) {
        if(!!pwtCntrl.last_user.entity[ref.modelAttr]){
          if (_.isArray(pwtCntrl.last_user.entity[ref.modelAttr])) {
            pwtCntrl.last_user.entity[ref.modelAttr] = _.map(pwtCntrl.last_user.entity[ref.modelAttr], 'uuid');
          } else {
            pwtCntrl.last_user.entity[ref.modelAttr] = pwtCntrl.last_user.entity[ref.modelAttr].uuid;
          }
        }
      });
...
//           'id' : pwtCntrl.last_user.entity.id,
      ...
      dataService.putOne(pwtCntrl.modelMeta.model, pwtCntrl.last_user.entity, config)
        .then(function(data) {
          ...
          var index = _.indexOf(pwtCntrl.items, _.find(pwtCntrl.items, {uuid: data.uuid}));
